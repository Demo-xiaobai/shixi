...
def warning(img,x,y):       # 画×
    ...
    cv2.circle(img, (x+25,y+25), 30, color, thickness)
    return img
...
def reverse_Track(model_path,video_path):
    ...
    while cap.isOpened():
        ...
        #设置感兴趣的ROI  
            ...
            results = model.track(roi,classes=[0,2,5,7])             # 跟踪
            ...
            for box in boxes:
                ...
                for pre_box in pre_boxs:    # 取出前一帧队列里中的一个检测框
                    ...
                    # 判断是否逆行
                    if np.sign(dY) == -1  and np.abs(dY)>=2:   #np.sign(dY) == -1 表示 < 0 逆行方向
                            # if (np.abs(dY)>=3 and np.abs(dY)<=10):
                        if np.abs(dX)>=-2 and np.abs(dX)<=6:
