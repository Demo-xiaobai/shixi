...
def track_goal(queue,model,img_queue,points):
    global run_track_thread
    # img_queue 队列中 cv线程的 所有图片
    while True:
        with lock:
            while not run_track_thread:
                condition.wait() # 这里执行cv线程
            while True:    
                        ...
                        for box in boxes:   # 遍历当前帧的所有检测框
                            ...
                            if cur_car_id == pre_car_id:
                                ...
                                if (reverse_vector[0] != 0) and (reverse_vector[1] != 0): # 车辆不是静止情况下计算夹角
                                    if(np.sign(dY.cpu())==-1) and ((np.abs(dY.cpu())>=2) and (np.abs(dY.cpu())<=50)):
                ...
                run_track_thread = False
                # 通知B线程可以执行
                condition.notify()

def get_image(model_path,video_path,img_queue):
    ''' 
        放入2帧图像到队列中
    ...
    while True:
        with lock:
            while run_track_thread:
                condition.wait() # 等待 track线程执行完毕 否之执行cv线程
...
if __name__ == '__main__':
    ...
    cv_thread = threading.Thread(target=get_image,args=(Yolo_path,video_path,img_queue))  # 读取视频帧的线程  返回出framek
    cv_thread.start()
    track_thread = threading.Thread(target=track_goal,args=(queue,model,img_queue,points)) # 追踪处理的函数
    track_thread.start()
    track_thread.join()
