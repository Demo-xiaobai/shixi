...
def track_goal(queue,model,points):
    ...
    while True:
        while not img_queue.empty():  # 存储每张图片的队列
                track_frame = img_queue.get() # 取一张图片 
                results = model.track(track_frame,classes=[0,2,5,7],persist=True,conf=0.6)
                ...
                while not queue.empty():
                    ...
                    for box in boxes:   # 遍历当前帧的所有检测框
                        ...
                        # print("mmmmmmmmmmmmm-----------------------")
                        if (cur_car_id == pre_car_id) and is_inside(cur_center_position[0][0],cur_center_position[0][1],points)==True:
                            dY = pre_center_position[0][1] - cur_center_position[0][1]
                            dX = pre_center_position[0][0] - cur_center_position[0][0] # x
                            reverse_vector = pre_center_position[0] - cur_center_position[0]# CD x1-x2=（0，0） 方向向下
                            if (reverse_vector[0] != 0) and (reverse_vector[1] != 0): # 车辆不是静止情况下计算夹角
                                if(torch.sign(dY)==-1) and ((torch.abs(dY)>=2) and (torch.abs(dY)<=25)):
                                    if(((torch.sign(dX)==1) or (torch.sign(dX)==0)) )and ((torch.abs(dX)>=0) and (torch.abs(dX)<=10)) :
                                    # if (np.abs(d))
                                        ...
                                        if (torch.abs(theta)>=0) and (torch.abs(theta)<=30):  #判定逆行
                ...
                cv2.polylines(annotated_frame, [points], isClosed=True, color=(255, 0, 0), thickness=3)       # 绘制蓝色的多边形检测区域 
