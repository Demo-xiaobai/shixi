...
def track_goal(model,points):
    global queue
    global frame_list                                               # 线程cv提取的每张图片
    normal_direction_points = np.array([[406,162],[360,376]])       # 顺行方向坐标   
    # normal_direction_points = np.array([[352,262],[452,262]])
    ...
    while True:
        if np.size(frame_list) != 0:
                results = model.track(frame_list,classes=[0,2,5,7],persist=True,conf=0.6)
                ...
                while not queue.empty():
                    ...
                    for box in boxes:   # 遍历当前帧的所有检测框
                        ...
                        if (cur_car_id == pre_car_id):
                            ...
                            # if(torch.sign(dX)==-1) and (torch.abs(dX)>=1)and(torch.abs(dX)<=40):
                            #     if((torch.abs(dY)>=2) and (torch.abs(dY)<=25)) :
                                ...
                                # 用公式计算向量夹角
                                    ...
                                    if (torch.abs(theta)>=0) and (torch.abs(theta)<=60):  #判定逆行
                                        cv2.putText(annotated_frame, 'WRONG DIRECTION', (int(cur_position[0][0]), int(cur_position[0][1])+100), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 3) 
                                        warning(annotated_frame,int(cur_position[0][0]+50),int(cur_position[0][1])+10) # 绘制警告图标
                        else:
                ...
                # 画出正确行驶路线
                cv2.line(annotated_frame,(406,162),(360,376),(0, 255, 0),3)
                # cv2.line(show_img,(352,262),(452,262),(0,255,0),3)               
                cv2.line(annotated_frame,(406,162),(382,172),(0, 255, 0),3)
                cv2.line(annotated_frame,(406,162),(430,172),(0, 255, 0),3)
...
def get_image(model_path,video_path):
    ...
    while cap.isOpened():
        ...
        if success:
                time.sleep(0.0003)  # 睡一会 等待track线程执行完毕
                frame_list = frame                        
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    cap.release()
                    exit()
